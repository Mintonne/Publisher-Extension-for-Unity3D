    ...
    <v-flex
      ...
      <v-expansion-panel popout>
        <v-expansion-panel-content
          ...
          <div slot="header">
            {{ review.heading }} - {{ review.packageName }} by {{ review.reviewer }}
          </div>
          <v-card>
            <v-card-text>
              <p>{{ review.body }}</p>
              <p class="text-xs-right ma-0">
                Rated <span
                  class="review-stars"
                  :title="review.stars + ' stars'"> {{ "&bigstar;".repeat(review.stars) }}</span> by {{ review.reviewer.length > 20 ? review.reviewer.substr(0, 20) + '...' : review.reviewer }}
                <v-icon
                  width="16"
                  height="16"
                  @click.native="OpenLink(review.link)">
                  $vuetify.icons.open
                </v-icon>
              </p>
            </v-card-text>
...
export default {
  ...
  activated () {
    if (!this.$store.getters.pubIdStatus) { return this.RedirectToSettings(this.$router) } else if (this.$store.getters.getReviewsFeed == null) { return this.RedirectToSettings(this.$router, false) }

    if (this.reviewsData == null) { this.GetReviewsData() }
  },
  methods: {
    GetReviewsData () {
      ...
      axios.get(link, {
        ...
        .then((response) => {
          ...
          for (let x = 0; x < itemCount; x++) {
            ...
            if (title.toLowerCase().includes('reply to review')) { continue }
            ...
            if (title.toLowerCase().includes('updated review')) { isUpdate = true }
