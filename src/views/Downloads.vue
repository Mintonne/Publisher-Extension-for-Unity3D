<template>
  <div class="section pb-3">
    ...
    <v-layout v-if="downloadsData != null && downloadsData.length > 0" row wrap>
      <package-tile v-for="(item, index) in packagesData" :key="index" :packageData="item" :term="searchTerm" downloads />
    </v-layout>

    <snackbar :value="showSnackbar" :callback="GetDownloadsData" timeout="5000" />
  </div>
</template>
...
export default {
  ...
  activated() {
    ...
    if (this.downloadsPeriod == null)
      this.downloadsPeriod = this.$store.getters.getCurrentMonth;
