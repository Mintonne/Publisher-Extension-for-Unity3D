...
chrome.runtime.onMessage.addListener(request => {
  if (request.command === 'restart') RestartChecker();
  if (request[updateFrequencyKey] != null) UpdateInterval(request[updateFrequencyKey]);
});

function StartChecker() {
  chrome.storage.local.get([pubIDKey, currentMonthsKey, updateFrequencyKey], results => {
    if (results == null || results[pubIDKey] == null || results[currentMonthsKey] == null) return;
...
chrome.windows.onRemoved.addListener(windowId => {
  ...
  if (windowId == curWindow.id) curWindow = null;
});
